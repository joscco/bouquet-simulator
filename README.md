...
For the current bouquet, choose a video format in the simulator menu and select **Loop-Video exportieren**. The default is square 1080×1080; Story 1080×1920 and landscape 1920×1080 are also available. The browser records six seconds directly from the 3D scene, including the current zoom and camera inclination, restores the original view afterwards, and preferably downloads a seamless H.264 MP4 loop. Browsers without MP4 recording support fall back to WebM. When embedding the file on a website, add the HTML `loop` attribute to the video element so playback repeats automatically.

The simulator's **Szene** controls store a light or neutral anthracite background and three independent light options for each bouquet: warm-white **Lichterglanz**, sparse animated **Lichtpunkte** with synchronized local illumination, and a warm **Licht von unten** made from three scale-aware ground spots aimed at staggered bouquet heights. Legacy firefly and glitter settings enable the first two options during migration. All animated lights use the same deterministic six-second cycle as the video export. The export samples the center of every frame interval so its first and last encoded frames are never duplicate loop endpoints. Dark scenes use directional blue moonlight instead of the daylight setup.
